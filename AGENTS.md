...
- This repo publishes a library Helm chart (`oci://ghcr.io/alexmkx/charts/frr-sidecar`) and an image (`ghcr.io/alexmkx/garuda-frr-sidecar`). The chart is consumed by other components via `dependencies:` with `repository: oci://ghcr.io/alexmkx/charts` and `dependency_update = true`. The chart `version` in `charts/frr-sidecar/Chart.yaml` MUST equal the git tag (`vX.Y.Z` ↔ `X.Y.Z`).
